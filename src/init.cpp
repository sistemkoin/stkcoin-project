...
void Shutdown()
{
    ...
    RenameThread("sistemkoin-shutoff");
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<http://stkcoin.io>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("sistemkoin-loadblk");
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if sistemkoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if sistemkoin is started while in a temporary directory.\n",
