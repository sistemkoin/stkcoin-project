...
const char * const BITCOIN_CONF_FILENAME = "sistemkoin.conf";
const char * const BITCOIN_PID_FILENAME = "sistemkoind.pid";
...
#else
    const char* pszModule = "sistemkoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Sistemkoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Sistemkoin";
#else
    // Unix
    return pathRet / ".sistemkoin";
#endif
#endif
}
